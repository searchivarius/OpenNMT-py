...
./translate.py -model "$MODEL_FULL_PATH" -src "$DATA_DIR/src-test.txt" -output "$PRED_FILE" -replace_unk -verbose
tools/multi-bleu-detok.perl "$DATA_DIR/tgt-test.txt" < "$PRED_FILE"